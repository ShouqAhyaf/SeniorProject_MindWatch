...
print("Using cuML RandomForest on GPU (EEG-only ch1_/ch2_).")
...
MODELS_DIR = Path("/mnt/c/Users/LENOVO/Downloads/models_nested_rf_nlnso_eeg_gpu")
...
def compute_inner_auc(
    ...
    print("\n------------------------------------------------------")
    print("Inner loop ROC AUC hyperparameter tuning (GPU)")
    print("------------------------------------------------------")
    ...
    for idx, params in enumerate(param_grid):
        ...
        print(f"Combo {idx+1}/{len(param_grid)} {params} "
...
# فقط الأعمدة اللي تبدأ بـ ch1_ أو ch2_ (case-insensitive)
feature_cols = [
...
for outer_idx, test_subj in enumerate(outer_folds, start=1):
    ...
    # -------------------------
    # Train FINAL model for this outer fold (GPU)
    # -------------------------
    ...
    # save model
    model_path = MODELS_DIR / f"rf_outer_{outer_idx:02d}_eeg_gpu.pkl"
    joblib.dump(model, model_path)
    print("Saved model:", model_path)

    # evaluation
    m = evaluate_metrics(y_test, y_pred, label_set)
    print_metrics(m, header=f"OUTER Fold {outer_idx} Test Metrics (EEG GPU)")
...
print_metrics(global_m, header="GLOBAL Metrics Across All 10 OUTER Folds (EEG GPU)")
...
auc_path = MODELS_DIR / "inner_auc_summary_eeg_gpu.csv"
...
print("\nSelected FINAL deployment hyperparameters (EEG GPU):", final_params)
...
final_path = MODELS_DIR / "rf_final_deployment_eeg_gpu.pkl"
joblib.dump(final_model, final_path)

print("\nFinal deployment model saved to:", final_path)
print("Use this model for real-time predictions (EEG-only, GPU).")
