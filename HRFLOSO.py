...
# 🔹 NEW: models directory name (5-subject CV, no nested LOSO)
MODELS_DIR = Path("/mnt/c/Users/LENOVO/Downloads/models_rf_5subj_allfeat_hybridsmote_gpu")
...
def rf_fit_predict_proba_gpu(X_train, y_train, X_val, params):
    ...
    (Used سابقًا في الـ inner loop - الآن مو مستخدمة، بس نخليها لو حبيتي ترجعين للنستد)
    """
...
# ============================================================
# 6) 5-subject grouped CV (LOSO-style on subjects)
# ============================================================

# نكوّن فولدات 5-subject CV بدل nested LOSO
...
for outer_idx, test_subj in enumerate(outer_folds, start=1):
    ...
    # -------------------------
    # Train FINAL model for this fold (GPU + Hybrid SMOTE)
    # -------------------------
    ...
    # 🔹 NEW model filename (5-subject CV, ALL features + Hybrid SMOTE)
    model_path = MODELS_DIR / f"rf_outer_{outer_idx:02d}_allfeat_hybridsmote_gpu_5subj.pkl"
    joblib.dump(model, model_path)
    print("Saved model:", model_path)

    # evaluation
    m = evaluate_metrics(y_test, y_pred, label_set)
    print_metrics(m, header=f"OUTER Fold {outer_idx} Test Metrics (ALL features, Hybrid SMOTE, GPU, 5-subject CV)")
...
print_metrics(global_m, header="GLOBAL Metrics Across All 5-subject CV Folds (ALL features, Hybrid SMOTE, GPU)")
...
auc_path = MODELS_DIR / "inner_auc_summary_allfeat_hybridsmote_gpu_5subj.csv"
...
print("\nSelected FINAL deployment hyperparameters (ALL features, Hybrid SMOTE, GPU, 5-subject CV):", final_params)
...
final_path = MODELS_DIR / "rf_final_deployment_allfeat_hybridsmote_gpu_5subj.pkl"
joblib.dump(final_model, final_path)

print("\nFinal deployment model saved to:", final_path)
print("Use this model for real-time predictions (ALL numeric features, Hybrid SMOTE, GPU, 5-subject CV).")